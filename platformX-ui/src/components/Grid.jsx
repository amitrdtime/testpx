...
const GridComponent = React.memo(
  ({ columns, data, basePath, onActionClick, clientId }) => {
    
    // State to manage sorting only for the displayName column
    const [sortOrder, setSortOrder] = useState("asc");
    const [sortField, setSortField] = React.useState('sowNo');
    ...
    const getColumnContent = (column, row) => {
      ...
      switch (column.id) {
        ...
        case "customerId":
          return (
            <TooltipLink
              to={`${basePath}/${clientId}/sows/${row.id || row.sowNo}`}
              title={value}
            >
              {value}
            </TooltipLink>
          ...
          case "projectNo":
            return (
              <TooltipLink
                to={`${basePath}/${clientId}/sows/${row.sowId}/projects/${row.id}`}
                title={value}
              >
                {value}
              </TooltipLink>
            ...
            <TableRow sx={{
              ...
              {columns.map((column) => (
                <TableCell
                  ...
                  sortDirection={
                    column.id === "displayName" ? sortOrder : false
                  }
                  sx={{ minWidth: 170 }}
                >
